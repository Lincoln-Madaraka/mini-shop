<div class="w-full h-screen overflow-x-hidden border-t flex flex-col">
    <main class="w-full flex-grow p-6">
        <h1 class="text-3xl font-bold text-gray-900 dark:text-gray-600 mb-6">Assign Product</h1>
        ...
        <form wire:submit.prevent="store" class="bg-white p-6 rounded shadow-md dark:bg-gray-800">
            ...
            <div class="grid gap-6 mb-6 md:grid-cols-2">
                ...
                <div>
                    <label for="gross_product" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Gross Product</label>
                    <input type="number" id="gross_product" wire:model="gross_product" min="0" step="any"
                    ...
                    @error('gross_product') <span class="text-red-600 text-sm">{{ $message }}</span> @enderror
                </div>
            </div>
...
<button type="submit"
    ...
    {{ $productIdBeingEdited ? 'Update Product' : 'Assign Product' }}
</button>
            ...
            <table class="min-w-full table-auto">
                <thead class="bg-gray-100 dark:bg-gray-100">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-sm font-semibold">Net Product</th>
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-600 g-gray-100 dark:bg-gray-100"">
                    @foreach ($salaries as $product)
                        <tr>
                            ...
                            <td class="px-6 py-4">Ksh {{ number_format($product->gross_product, 2) }}</td>
                            <td class="px-6 py-4">Ksh {{ number_format($product->shif, 2) }}</td>
                            <td class="px-6 py-4">Ksh {{ number_format($product->housing_levy, 2) }}</td>
                            <td class="px-6 py-4">Ksh {{ number_format($product->paye, 2) }}</td>
                            <td class="px-6 py-4 font-semibold text-green-700">Ksh {{ number_format($product->net_product, 2) }}</td>
